...
104.27.198.101    4       4       0.00    129.52    12.71
104.22.43.157     4       4       0.00    131.38    16.74
...
# 如果你对下载速度比较在意，一般只需要指定 下载速度下限 就行了，延迟时间上限 不是很有必要，因为本身就是以
# 延迟高低顺序 去测试下载速度的（所以得到的结果本身就是：即满足下载速度，又延迟最低），而且延迟高低峰期
# 差别较大（我这里 120+~200+），所以在设置的值较低时自动化测速，可能会出现匹配不到的情况。
```
