...
:: 用户自定义 Hosts 文件路径（如果为空，则使用默认系统hosts路径C:\Windows\System32\drivers\etc）
:: 示例：set HOSTS_PATH=D:\CloudflareST\custom_cf_hosts.txt
set HOSTS_PATH=
...
:: 确保路径的文件夹存在
for %%i in ("%HOSTS_PATH%") do (
    mkdir "%%~dpi" >nul 2>nul
)

:: 如果 HOSTS_PATH 指定的文件不存在，则创建一个空的文本文件
if not exist "%HOSTS_PATH%" (
    echo. > "%HOSTS_PATH%"
    echo 已创建空的 Hosts 文件：%HOSTS_PATH%
)
...
echo.
echo 开始备份 Hosts 文件...
copy "%HOSTS_PATH%" "%HOSTS_PATH%_backup" >nul 2>nul
echo 开始替换...
(
    for /f "tokens=*" %%i in ('type "%HOSTS_PATH%_backup"') do (
        set s=%%i
